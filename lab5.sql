...
as 
	if exists(select*from PHONGBAN where MAPHG = @MAPHG)
		update PHONGBAN set TENPHG = @TENPHG, TRGPHG = @TRPHG, NG_NHANCHUC = @NG_NHANCHUC
			where MAPHG = @MAPHG
	else
		insert into PHONGBAN 
		values(@TENPHG,@MAPHG,@TRPHG,@NG_NHANCHUC);
		drop proc sp_themphongbanmoi

exec sp_themphongbanmoi 'CNTT',15, '005', '12-10-2020';
...
exec sp_capnhatphongban 'CNTT', 'IT', 15, '005', '1-1-2020';
